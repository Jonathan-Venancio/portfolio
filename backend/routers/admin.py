...
# Profile CRUD
@router.get("/profile")
async def get_admin_profile(db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
...
@router.put("/profile")
async def update_admin_profile(profile_data: ProfileCreate, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
    profile = crud.get_profile(db)
    if not profile:
...
async def upload_profile_image(file: UploadFile = File(...), current_user: str = Depends(auth.get_current_user)):
...
async def update_admin_category(category_id: int, category: CategoryCreate, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
    return crud.update_category(db, category_id, category)

@router.delete("/categories/{category_id}")
async def delete_admin_category(category_id: int, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
...
async def update_admin_project(project_id: int, project: ProjectCreate, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
    return crud.update_project(db, project_id, project)

@router.delete("/projects/{project_id}")
async def delete_admin_project(project_id: int, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
...
async def update_admin_skill(skill_id: int, skill: SkillCreate, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
    return crud.update_skill(db, skill_id, skill)

@router.delete("/skills/{skill_id}")
async def delete_admin_skill(skill_id: int, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
...
async def update_admin_contact(contact_id: int, contact: ContactCreate, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
    return crud.update_contact(db, contact_id, contact)

@router.delete("/contacts/{contact_id}")
async def delete_admin_contact(contact_id: int, db: Session = Depends(get_db), current_user: str = Depends(auth.get_current_user)):
